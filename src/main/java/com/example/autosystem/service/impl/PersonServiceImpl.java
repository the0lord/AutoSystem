...
public class PersonServiceImpl implements PersonService {
    ...
    public PersonDto updatePersonAllInfo(Long personId, PersonDto personDto) {
        PersonDto returnValue = new PersonDto();
        Optional<Person> person = personRepository.findById(personId);
        Optional<Group> group =groupRepository.findById(personDto.getGroupId().getId());
        if (person.isEmpty()) {
            throw new RuntimeException("Person doesn't exists");
        }
        ...
        person.get().setGroupId(group.get().getPerson().getGroupId());
        ...
        person.get().setTrainer(personDto.getTrainer());
